...
const socket = io("https://design-appointments-camping-dietary.trycloudflare.com", {
...
const useSocketIO = () => {
  ...
  // ✅ NEW: helper to subscribe to socket events
  const onEvent = useCallback((event, callback) => {
    socket?.on(event, callback);
  }, []);

  // ✅ NEW: helper to unsubscribe from socket events
...
const useVideoGeneration = (shopify) => {
  ...
  const { connected, emitEvent, onEvent, offEvent } = useSocketIO();

  // Handle creation updates from backend via Socket.IO
  const handleCreationUpdate = useCallback((data) => {
    const { creationId, taskId, status, outputMap, failureReason } = data;
  ...
  useEffect(() => {
    // Subscribe to Socket.IO events from backend
    onEvent("creationUpdate", handleCreationUpdate);
    return () => offEvent("creationUpdate", handleCreationUpdate);
  }, [handleCreationUpdate, onEvent, offEvent]);
  ...
  const generateVideo = useCallback(async (params) => {
    try {
      ...
      const freepikPayload = {
        webhook_url: "https://your-domain.com/api/freepik/webhook", // Optional
        image: params.image,
      ...
      // Step 1: Ask backend to start Freepik generation
      const response = await fetch("/api/v1/app/freepik/generate-video", {
        method: "POST",
      ...
      console.log("Video generation task started with ID:", taskId);
      setCurrentTaskId(taskId);
      setGenerationProgress(5);

      // Step 2: Create a creation record in your backend
      ...
      if (!creationRes.ok) throw new Error("Backend creation failed");

      const creationData = await creationRes.json();
      const creationId = creationData._id || creationData.id;
      setCurrentCreationId(creationId);

      // Step 3: Subscribe via Socket.IO
      if (connected) {
        emitEvent("subscribe_creation", { type: "subscribe_creation", creationId, taskId });
      }
    } catch (error) {
  ...
  }, [connected, emitEvent, shopify]);

  const cancelGeneration = useCallback(() => {
    if (currentCreationId && connected) {
    ...
    setCurrentTaskId(null);
    setGenerationResult(null);
  }, [currentCreationId, currentTaskId, emitEvent, connected]);
