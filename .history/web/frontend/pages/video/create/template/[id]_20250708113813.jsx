...
// API Configuration (FIXED - removed websocketUrl)
const API_CONFIG = {
  baseUrl: "/api/v1/app",
};
...
const useVideoGeneration = (shopify) => {
  ...
  // FIXED: Use Socket.IO instead of custom WebSocket
  ...
  const generateVideo = useCallback(
    async (params) => {
      try {
        ...
        const creationPayload = {
          ...
          type: "video",
          task_id: task_id, // this is task id from freepik API{"data": {"task_id": "046b6c7f-0b8a-43b9-b35d-6489e6daee91","status": "IN_PROGRESS"}
        ...
        // FIXED: Subscribe to creation updates via Socket.IO (not custom WebSocket)
        if (connected) {
    ...
    [connected, emitEvent, shopify, generationResult, isGenerating] // FIXED: dependencies
  );

  // FIXED: Cancel generation using Socket.IO
  const cancelGeneration = useCallback(() => {
    ...
    setCurrentCreationId(null);
    setGenerationResult(null);
  }, [currentCreationId, emitEvent]); // FIXED: dependencies
  ...
  return {
    ...
    generationProgress,
    currentCreationId,
    connectionStatus: connected ? "Connected" : "Disconnected", // FIXED: use Socket.IO status
  };
};
